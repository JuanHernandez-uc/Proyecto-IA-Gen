...
def query(limit, offset):
    ...
    WHERE {
    {
        ...
        ?persona wdt:P31 wd:Q5;   # Instancia de ser humano
                wdt:P27 wd:Q783.  # Filtra por la ciudadanía (país al que pertenece la entidad)
        } 
    """
    ...
    def expand_level(qids, depth=1, extension_limit = 5):
        all_triplets = []
    ...
    file_path = "Datasets/honduras.csv"

    if os.path.exists(file_path):
        df_prev = pd.read_csv(file_path)
        df_all = pd.concat([df_prev, df_all], ignore_index=True)
        df_all = df_all.drop_duplicates(subset=["entidad", "relacion", "objeto"])
        print(f"Archivo existente: {len(df_prev)} filas → Nuevo total: {len(df_all)} filas.")
    else:
...
for i in range(820, 1200, entidades):
    print("Ejecución:", j)
    try:
        query(entidades, i)
        j += 1
